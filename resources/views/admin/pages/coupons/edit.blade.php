    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header-coupon">
                <h5 class="page-title fs-5 fw-semibold">تعديل الكوبون: {{ $coupon->code }}</h5>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="coupon-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="coupon-form-section">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control @error('code') is-invalid @enderror" name="code" value="{{ old('code', $coupon->code) }}" required dir="ltr" style="text-align: left;">
                                    @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">اسم الكوبون <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $coupon->name) }}" required>
                                    @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-12">
                                    <label class="form-label">الوصف <span class="text-muted">(اختياري)</span></label>
                                    <textarea name="description" class="form-control" rows="2" placeholder="وصف مختصر للكوبون">{{ old('description', $coupon->description) }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <select name="type" class="form-select" required>
                                        ...
                                        <option value="fixed_amount" {{ old('type', $coupon->type) == 'fixed_amount' ? 'selected' : '' }}>مبلغ ثابت (ر.س)</option>
                                        <option value="buy_x_get_y" {{ old('type', $coupon->type) == 'buy_x_get_y' ? 'selected' : '' }}>اشترِ واحصل على</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">القيمة <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" min="0" class="form-control" name="value" value="{{ old('value', $coupon->value) }}" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">حد أدنى للطلب <span class="text-muted">(اختياري)</span></label>
                                    <input type="number" min="0" step="0.01" class="form-control" name="minimum_order_amount" value="{{ old('minimum_order_amount', $coupon->minimum_order_amount ?? 0) }}">
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="d-flex flex-wrap gap-3">
                                        <label class="form-check">
                                            <input type="radio" name="applicable_to" value="entire_store" class="form-check-input" {{ old('applicable_to', $coupon->applicable_to ?? 'entire_store') === 'entire_store' ? 'checked' : '' }}>
                                            <span class="form-check-label">عام على كل المتجر</span>
                                        </label>
                                        <label class="form-check">
                                            <input type="radio" name="applicable_to" value="specific_products" class="form-check-input" {{ old('applicable_to', $coupon->applicable_to ?? '') === 'specific_products' ? 'checked' : '' }}>
                                            <span class="form-check-label">منتجات محددة</span>
                                        </label>
                                        <label class="form-check">
                                            <input type="radio" name="applicable_to" value="specific_categories" class="form-check-input" {{ old('applicable_to', $coupon->applicable_to ?? '') === 'specific_categories' ? 'checked' : '' }}>
                                            <span class="form-check-label">تصنيفات محددة</span>
                                        </label>
                                    </div>
                                </div>
                                @php
                                    $selProductIds = old('product_ids', $coupon->products->pluck('id')->toArray());
                                    $selCategoryIds = old('category_ids', $coupon->categories->pluck('id')->toArray());
                                    $applicableTo = old('applicable_to', $coupon->applicable_to ?? 'entire_store');
                                @endphp
                                <div class="col-12" id="coupon-products-wrap" style="display: {{ $applicableTo === 'specific_products' ? 'block' : 'none' }};">
                                    ...
                                    <select name="product_ids[]" id="coupon-product-ids" class="form-select" multiple size="8">
                                        @foreach($products as $p)
                                            <option value="{{ $p->id }}" {{ in_array($p->id, $selProductIds) ? 'selected' : '' }}>{{ $p->name }}</option>
                                        @endforeach
                                    </select>
                                    <p class="hint-text">استخدم Ctrl أو Shift لاختيار أكثر من منتج</p>
                                </div>
                                <div class="col-12" id="coupon-categories-wrap" style="display: {{ $applicableTo === 'specific_categories' ? 'block' : 'none' }};">
                                    ...
                                    <select name="category_ids[]" id="coupon-category-ids" class="form-select" multiple size="8">
                                        @foreach($categories as $c)
                                            <option value="{{ $c->id }}" {{ in_array($c->id, $selCategoryIds) ? 'selected' : '' }}>{{ $c->parent_id ? '— ' : '' }}{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="coupon-form-section">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <input type="number" min="0" class="form-control" name="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}">
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control" name="starts_at" value="{{ old('starts_at', $coupon->starts_at?->format('Y-m-d')) }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">تاريخ الانتهاء <span class="text-muted">(اختياري)</span></label>
                                    <input type="date" class="form-control" name="expires_at" value="{{ old('expires_at', $coupon->expires_at?->format('Y-m-d')) }}">
                                </div>
                            </div>
                        </div>
