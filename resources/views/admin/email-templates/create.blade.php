    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div>
                    <h4 class="mb-0">إضافة قالب إيميل</h4>
                    <p class="mb-0 text-muted">قالب جديد للإشعارات — استخدم المتغيرات مثل <code>&#123;&#123; order.id &#125;&#125;</code> و <code>&#123;&#123; customer.name &#125;&#125;</code></p>
                </div>
                <div class="ms-auto">
            ...
            <form method="POST" action="{{ route('admin.email-templates.store') }}" id="email-template-form">
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control @error('key') is-invalid @enderror" name="key" value="{{ old('key') }}" required placeholder="مثال: order_created_ar">
                                        @error('key')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">الاسم المعروض <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required placeholder="مثال: إيميل إنشاء طلب - عربي">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="locale" class="form-select @error('locale') is-invalid @enderror" required>
                                            @foreach($locales as $code => $label)
                                                <option value="{{ $code }}" {{ old('locale', 'ar') === $code ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" class="form-control @error('subject') is-invalid @enderror" name="subject" value="{{ old('subject') }}" required placeholder="مثال: تم إنشاء طلبك #&#123;&#123; order.id &#125;&#125;">
                                        @error('subject')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label">وصف (اختياري)</label>
                                        <input type="text" class="form-control" name="description" value="{{ old('description') }}" placeholder="للاستخدام الداخلي فقط">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="tab-content">
                                    <div class="tab-pane fade show active" id="tab-editor" role="tabpanel">
                                        @php
                                    $defaultBodyHtml = '<p>مرحباً {{ customer.name }},</p><p>تم إنشاء طلبك رقم {{ order.id }}.</p>';
                                @endphp
                                        <textarea name="body_html" id="body_html" class="form-control @error('body_html') is-invalid @enderror" rows="12">{{ old('body_html', $defaultBodyHtml) }}</textarea>
                                        @error('body_html')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="tab-pane fade" id="tab-html" role="tabpanel">
                                        <textarea id="body_html_raw" class="form-control font-monospace" rows="16" dir="ltr"></textarea>
